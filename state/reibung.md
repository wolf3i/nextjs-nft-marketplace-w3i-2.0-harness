...
| 2026-09-09 | Abhängigkeits-Sanierung war als kleines Aufräumen eingeplant; der Dry-Run zeigte einen Umbau (141 geändert, 96 entfernt, 35 neu — inkl. `vite` 7→8 und `vitest` gegen den exakten Coverage-Pin). Die Fehleinschätzung kostete eine Planungsrunde: statt eines Fixes wurde eine belegte Zurückstellung nötig | `package.json` / `.github/workflows/ci.yml`, `npm audit fix --dry-run` | ~1 Planungsrunde | ja, als Entscheidung belegt in `docs/adr/0002-abhaengigkeiten-zurueckgestellt.md` statt still gelockert. Nachlauf: der eingeplante Nebenschritt `npx update-browserslist-db@latest` wurde aus demselben Grund zurückgerollt — er fasst nicht nur `package-lock.json` an, sondern entfernt über ein internes `npm uninstall` auch die exakte devDependency `baseline-browser-mapping 2.9.11` aus `package.json` und fällt damit selbst in den zurückgestellten Abhängigkeits-Umbau |
